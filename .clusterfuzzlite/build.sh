...
# Build the project with H2 instead of PostgreSQL.
# quarkus.datasource.db-kind is a build-time property — it must be set during
# compilation, not just when the resulting jar is started.
./mvnw package -DskipTests -Djacoco.skip=true \
  -Dquarkus.datasource.db-kind=h2 \
  -Dquarkus.datasource.jdbc.url='jdbc:h2:mem:fuzztest;DB_CLOSE_DELAY=-1' \
...
for fuzzer in $(grep -rl "fuzzerTestOneInput" src/test/java/ || true); do
  ...
  if [ "$simple_name" = "EndpointFuzzer" ]; then
    ...
    # Starts Quarkus as a subprocess (java -jar quarkus-run.jar) before launching
    # jazzer_driver. H2 was baked in at compile time; the runtime settings below
    # supply what application.properties deliberately leaves empty for production
    # (pepper, notify credentials, outbound URLs) with throwaway fuzz values.
    #
    # flyway.migrate-at-start is forced back on: the jar runs under the prod
    # profile, where application.properties disables it, but the in-memory H2
    # database starts empty and hibernate's schema validation would fail without
    # the migrations having run.
    #
    # The rest-client URLs point at a closed port on purpose: fuzzing must not
    # reach a real Profielservice or NotifyNL, and an outbound call that fails
    # fast still exercises the error paths in the service layer.
...
"$this_dir/open-jdk-25/bin/java" \
  ...
  -Dquarkus.rest-client.profielservice.url=http://localhost:9999 \
  -Dquarkus.rest-client.notify.url=http://localhost:9999 \
  -Dnotify.api-key=fuzz-api-key-niet-voor-productie \
  -Dnotify.callback.bearer-token=fuzz-callback-token \
  -Dhash.pepper=fuzz-pepper-niet-voor-productie \
  -Dlogboekdataverwerking.enabled=false \
  -jar "$this_dir/quarkus-app/quarkus-run.jar" &
QUARKUS_PID=$!
trap "kill $QUARKUS_PID 2>/dev/null || true" EXIT

# Wait for Quarkus to accept connections (up to 20 seconds). Readiness includes
# the datasource check, so this also confirms H2 + flyway came up.
for i in $(seq 1 80); do
  if curl -sf -o /dev/null http://localhost:8081/q/health/ready 2>/dev/null; then
    break
  fi
  sleep 0.25
done
