...
- **`StatusWaarde`** kent `CREATED`, `SENDING`, `DELIVERED`, `PERMANENT_FAILURE`,
  ...
  `V1__init_notificatie.sql` en `V2__notificatie_retentie.sql` sommen dezelfde waarden
...
- **Gebeurtenistijd en registratietijd zijn aparte kolommen.**
  ...
  `laatste_status_update`); `tijdstip` is het tijdstip voor het afleverbewijs en wordt
...
- **Expand/contract bij een kolomwijziging.** Voeg eerst de nieuwe kolom toe, vul hem
  ...
  meer leest. `V2__notificatie_retentie.sql` doet dat voor `notificatie.status` en
